...
--       where
--         go : Γ ↝⟨ Txout ⟩ Γ′
--         go txout′ rewrite namesʳ≡ = txout′

--     sechash↝ : R ↝⟨ Sechash ⟩ R′
--     sechash↝ = ↑ˢ go
--       where
--         go : Γ ↝⟨ Sechash ⟩ Γ′
--         go sechash′ rewrite namesˡ≡ = sechash′

--     κ↝ : R ↝⟨ 𝕂² ⟩ R′
--     κ↝ = ↑ᵏ go
--       where
--         go : Γ ↝⟨ 𝕂²ᶜ ⟩ Γ′
--         go κ′ rewrite L.++-identityʳ (authorizedHonAds Γ) = κ′

--     -- [BUG] cannot use names⊆ as an index :(
--     -- module H₃″ Γ₀ (Γ≡ : Γ ≡ (` ad ∣ Γ₀)) (names⊆ : names G ⊆ names Γ₀) where
--     module H₃″ Γ₀ (Γ≡ : Γ ≡ (` ad ∣ Γ₀)) where

--       -- txout↝′ : Txout R → Txout G
--       -- txout↝′ txout′ rewrite cfg≡ | Γ≡ = weaken-↦ txout′ (mapMaybe-⊆ isInj₂ names⊆)

--       txout↝′ : Txout R → Txout Γ₀
--       txout↝′ txout′ rewrite cfg≡ | Γ≡ = txout′

--       -- sechash↝′ : Sechash R → Sechash G
--       -- sechash↝′ sechash′ rewrite cfg≡ | Γ≡ = weaken-↦ sechash′ (mapMaybe-⊆ isInj₁ names⊆)

--       sechash↝′ : Sechash R → Sechash Γ₀
--       sechash↝′ sechash′ rewrite cfg≡ | Γ≡ = sechash′
...
-- -- module H₄ G C v z Γ₀ (ds : List DepositRef) partG where
-- --   private
-- --     ad = ⟨ G ⟩ C

-- --     -- [BUG] cannot get this to work here without explicitly passing ⦃ HPᵖ ⦄
-- --     -- partG = nub-participants {X = Precondition} G
...
-- --     module _ (honG : Any (_∈ Hon) partG) where

-- --       authH : ∀ {cs : List Configuration}
-- --         → Any (λ c → ad ∈ authorizedHonAds c) cs
-- --         → ad ∈ authorizedHonAds (|| cs)
-- --       authH {cs = c ∷ []} p with p
-- --       ... | here ad∈ = ad∈
...
-- --       ad∈₀ : ad ∈ authorizedHonAds Γ₃
-- --       ad∈₀ = authH h′
-- --         where
-- --           h : ∀ {p} → p ∈ Hon → ad ∈ authorizedHonAds (p auth[ ♯▷ ad ])
-- --           h {p} p∈ rewrite dec-true (p ∈? Hon) p∈ = here refl

-- --           h′ : Any ((ad ∈_) ∘ authorizedHonAds) (map (_auth[ ♯▷ ad ]) partG)
-- --           h′ = L.Any.map⁺ {f = _auth[ ♯▷ ad ]} (L.Any.map h honG)

-- --       ad∈ : ad ∈ advertisements R
-- --       ad∈ = {!!}
-- --       -- rewrite cfg≡ = ∈-++⁺ʳ (authorizedHonAds $ Γ₁ ∣ Γ₂) ad∈₀
