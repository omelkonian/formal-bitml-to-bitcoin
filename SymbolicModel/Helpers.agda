...
private module I = Induction
subtermsᵈ subtermsᵈ⁺ : Contracts → List Contract
subtermsᵈ = subterms′ ∘ I.CS
subtermsᵈ⁺ = subterms⁺ ∘ I.CS
...
𝕂′ : Advertisement → Set
𝕂′ (⟨ G ⟩ C) = subtermsᵈ C ↦ (nub-participants G ↦ KeyPair)
...
module H₂ Γ B A ad (Δ : List (Secret × Maybe ℕ)) where
  ...
  module H₂′ R R′ (cfg≡ : cfg (lastCfg R) ≡ Γ) (cfg≡′ : cfg (lastCfg R′) ≡ Γ′) where
    ...
    κ↝ : 𝕂 R → (k⃗ : subtermsᵈ (C ad) ↦ (nub-participants (G ad) ↦ KeyPair)) → 𝕂 R′
